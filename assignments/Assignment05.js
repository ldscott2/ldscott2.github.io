...
var chartData = {
  ...
  data: {
    ...
    datasets: [
      ...
      {
        label: "TotalCasesPer100000",
        data: [],
        backgroundColor: "rgba(0,179,255,0.4)"
      }
    ]
  },
  options: {
    ...
    if (this.readyState == 4 && this.status == 200) {
      ...
      for (let c of covidJsObj.Countries) {
        if (c.TotalDeaths > 50000) {
          TotalConfirmedCovid.push({
            Slug: c.Slug,
            TotalConfirmed: c.TotalConfirmed,
            TotalDeaths: c.TotalDeaths,
            Populations: populations[c.Slug],
            TotalConfirmedPer100000: populations[c.Slug] / 100000
          });
        }
      }
      TotalConfirmedCovid = _.orderBy(
        TotalConfirmedCovid,
      ...
      chartData.data.datasets[2].backgroundColor = "rgba(0,179,255,0.4)";
      ...
      chartData.data.labels = TotalConfirmedCovid.map((x) => x.Slug);
      chartData.data.datasets[0].data = TotalConfirmedCovid.map(
        (x) => x.TotalConfirmed
      );
      chartData.data.datasets[1].data = TotalConfirmedCovid.map(
        (x) => x.TotalDeaths
      );
      chartData.data.datasets[2].data = TotalConfirmedCovid.map(
