...
function displayAbout() {
  window.location.href = "ClubMeAbout.html";
...
// navigate to "clubDistanceList.html"
...
function populateStatsTable() {
  document.getElementById("cmn_club").innerHTML =
    "<strong>" + clubs[clubRow][1] + "</strong>";
  document.getElementById("cmn_min").innerHTML = Math.round(clubs[clubRow][4]);
  document.getElementById("cmn_avg").innerHTML =
    "<strong>" + Math.round(clubs[clubRow][3]) + "</strong>";
...
function appendTapEntryButtons() {
  ...
  for (let i = avgDistPlusSome; i > avgDistMinusSome; i -= 5) {
    ...
    btn.innerHTML =
      "<button class='cmn_noPadding cmn_fullHeight cmn_tapEntry' onclick='updateStats(" +
      i +
      ");'>" +
      i +
      "</button> ";
...
function updateStats(shotDistance = 0) {
  ...
  if (parseInt(shotDistance) > 0) {
    ...
    newAverage =
      (currentAverage * currentNumShots + shotDistance) / (currentNumShots + 1);
